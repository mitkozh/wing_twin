...
class DataLoader:
    ...
    def load(self) -> Optional[dict]:
        """Load recording as a flat dict of HDF5 paths to numpy arrays."""
    ...
    def load_tuple(self) -> RecordingData:
        """Load recording and return structured RecordingData."""
        data = self.load()
        if data is None:
...
def _load_cycles(data: dict, data_dir: Path) -> list:
    """Load cycles from HDF5 (legacy) or fatigue_state.json."""
